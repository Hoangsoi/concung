...
export async function POST(request: Request) {
  try {
    ...
    try {
      // 1. Ensure admins table exists in Neon PostgreSQL
      await sql`
      ...
      // 2. Ensure default admin user exists in Neon PostgreSQL
      await sql`
      ...
      // 3. Query admin user from Neon
      const rows = await sql`
        SELECT id, username, password FROM admins WHERE username = ${cleanUsername} LIMIT 1;
      `;
    ...
    // Fallback credential check if Neon DB query is temporarily unreachable
